...
export const rowsWithExtra = [];

// eslint-disable-next-line consistent-return
const getRows = async () => {
  try {
    const res = await axios.post("http://localhost:443/api/study/list", {});
    const data = res.data?.data || [];
    const newData = await data.map((e) => {
      return {
        employeeId: e.prot_nbr,
        name: e.prot_nbr_stnd,
        dept: e.phase,
        email: e.prot_status,
        employmentStatus: e.spnsr_nm,
        hireDate: "07/08/2016",
      };
    });
    rows.push(newData);

    const newWithData = await newData.map((row) => ({
      ...row,
    ...
    rowsWithExtra.push(newWithData);
